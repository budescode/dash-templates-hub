...
def update_dashboard(state, city):
    ...
    map_df = df.dropna(subset=["geolocation_lat", "geolocation_lng", "geolocation_state"])
    colors = px.colors.qualitative.Plotly
    figure_scatter = go.Figure()
    for i, state in enumerate(map_df["geolocation_state"].unique()):
        sdf = map_df[map_df["geolocation_state"] == state]
        figure_scatter.add_trace(go.Scattermapbox(
