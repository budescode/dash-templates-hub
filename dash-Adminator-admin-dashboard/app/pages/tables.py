...
def _table(rows, className="table", head_dark=False):
    thead_class = "table-dark" if head_dark else ""
    return html.Table(
        className=className,
        children=[
            html.Thead(
                html.Tr([html.Th("#"), html.Th("First Name"), html.Th("Last Name"), html.Th("Username")]),
                className=thead_class
            ),
            html.Tbody([
                html.Tr([html.Th(str(r[0]), scope="row"), html.Td(r[1]), html.Td(r[2]), html.Td(r[3])])
                for r in rows
            ])
        ]
    )
...
_rows = [(1, "Mark", "Otto", "@mdo"), (2, "Jacob", "Thornton", "@fat"), (3, "Larry", "the Bird", "@twitter")]


def create_basic_table_page():
    def card(title, desc, table_el):
        return html.Div(
            className="col-md-12",
            children=html.Div(
                className="bgc-white bd bdrs-3 p-20 mB-20",
                children=[
                    html.H4(title, className="c-grey-900 mB-20"),
                    html.P(desc),
                    table_el
                ]
            )
    ...
    return html.Div(
        ...
        children=[
            html.H4("Basic Tables", className="c-grey-900 mT-10 mB-30"),
            ...
            html.Div(className="row", children=[
                card("Table head options",
                     "Use .table-light or .table-dark to make <thead>s appear light or dark gray.",
                     _table(_rows, head_dark=True))
            ]),
            html.Div(className="row", children=[
                card("Striped rows",
                     "Use .table-striped to add zebra-striping to any table row within the <tbody>.",
                     _table(_rows, className="table table-striped"))
            ]),
            html.Div(className="row", children=[
                card("Bordered table",
                     "Add .table-bordered for borders on all sides of the table and cells.",
                     _table(
                         [(1, "Mark", "Otto", "@mdo"), (2, "Mark", "Otto", "@TwBootstrap"),
                          (3, "Jacob", "Thornton", "@fat"), (4, "Larry the Bird", "", "@twitter")],
            ...
            html.Div(className="row", children=[
                html.Div(
                    ...
                    children=html.Div(
                        ...
                        children=[
                            html.H4("Hoverable rows", className="c-grey-900 mB-20"),
                            html.P("Add .table-hover to enable a hover state on table rows within a <tbody>."),
                            html.Table(
                                className="table table-hover",
                                children=[
                                    html.Thead(html.Tr([html.Th("#"), html.Th("First Name"), html.Th("Last Name"), html.Th("Username")])),
                                    html.Tbody([
                                        html.Tr([html.Th("1", scope="row"), html.Td("Mark"), html.Td("Otto"), html.Td("@mdo")]),
                                        html.Tr([html.Th("2", scope="row"), html.Td("Jacob"), html.Td("Thornton"), html.Td("@fat")]),
                                        html.Tr([html.Th("3", scope="row"), html.Td("Larry the Bird", colSpan=2), html.Td("@twitter")]),
                                    ])
                                ]
                            )
                        ]
...
_DT_COLUMNS = [{"name": c, "id": c} for c in ["Name", "Position", "Office", "Age", "Start date", "Salary"]]


def create_datatable_page():
    return html.Div(
        className="container-fluid",
        children=[
            html.H4("Data Tables", className="c-grey-900 mT-10 mB-30"),
            html.Div(className="row", children=[
                html.Div(className="col-md-12", children=html.Div(
                    className="bgc-white bd bdrs-3 p-20 mB-20",
                    children=[
                        html.H4("Bootstrap Data Table", className="c-grey-900 mB-20"),
